...
import { exchangeCodeForToken, fetchProfile, parseState } from '@/lib/line/auth'
...
export async function GET(req: NextRequest) {
  ...
  const storedNonce = req.cookies.get('line_oauth_state')?.value
  const { nonce, redirectPath } = parseState(state)
