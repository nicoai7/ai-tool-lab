...
export default function GenderPage() {
  ...
  const { data, loading, isDemo } = useBreakdownData('gender', dateRange);
  ...
  return (
    <div>
      <PageHeader title="広告レポート - 性別" startDate={startDate} endDate={endDate} onDateChange={onChange} isDemo={isDemo} loading={loading} />
      <div className="flex justify-end mb-3">
        <ExportButton
          onCSV={() => exportBreakdownToCSV(data, '性別', '広告レポート_性別')}
          onExcel={() => exportBreakdownToExcel(data, '性別', '広告レポート_性別', '性別')}
          onPptx={() => exportBreakdownToPptx(data, '広告レポート - 性別', '性別', '広告レポート_性別')}
        />
      </div>
      <div className="bg-card-bg rounded-xl border border-border overflow-hidden mb-8">
        <DataTable data={data} labelHeader="性別" />
      </div>
