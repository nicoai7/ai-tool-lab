...
export default function WeekdayReportPage() {
  const { dateRange, onChange, startDate, endDate } = useDateRange();
  const { data, loading, isDemo } = useBreakdownData('weekday', dateRange);
  ...
  return (
    <div>
      <PageHeader title="広告レポート - 曜日" startDate={startDate} endDate={endDate} onDateChange={onChange} isDemo={isDemo} loading={loading} />

      <div className="flex justify-end mb-3">
        <ExportButton
          onCSV={() => exportBreakdownToCSV(data, '曜日', '広告レポート_曜日別')}
          onExcel={() => exportBreakdownToExcel(data, '曜日', '広告レポート_曜日別', '曜日別')}
          onPptx={() => exportBreakdownToPptx(data, '広告レポート - 曜日別', '曜日', '広告レポート_曜日別')}
        />
      </div>

      <div className="bg-card-bg rounded-xl border border-border overflow-hidden mb-8">
        <DataTable data={data} labelHeader="曜日" />
      </div>
