...
export default function MonthlyReportPage() {
  ...
  const { data, loading, isDemo } = useBreakdownData('monthly', dateRange);
  ...
  return (
    <div>
      <PageHeader title="広告レポート - 月別" startDate={startDate} endDate={endDate} onDateChange={onChange} isDemo={isDemo} loading={loading} />

      <div className="flex justify-end mb-3">
        <ExportButton
          onCSV={() => exportBreakdownToCSV(data, '年月', '広告レポート_月別')}
          onExcel={() => exportBreakdownToExcel(data, '年月', '広告レポート_月別', '月別')}
          onPptx={() => exportBreakdownToPptx(data, '広告レポート - 月別', '年月', '広告レポート_月別')}
        />
      </div>

      <div className="bg-card-bg rounded-xl border border-border overflow-hidden mb-8">
        <DataTable data={data} labelHeader="年月" />
      </div>
