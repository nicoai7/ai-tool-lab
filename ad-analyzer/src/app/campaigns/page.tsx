...
export default function CampaignsPage() {
  ...
  const { data, loading, isDemo } = useBreakdownData('campaign', dateRange);
  ...
  return (
    <div>
      <PageHeader title="広告レポート - キャンペーン" startDate={startDate} endDate={endDate} onDateChange={onChange} isDemo={isDemo} loading={loading} />
      <div className="flex justify-end mb-3">
        <ExportButton
          onCSV={() => exportBreakdownToCSV(data, 'キャンペーン', '広告レポート_キャンペーン別')}
          onExcel={() => exportBreakdownToExcel(data, 'キャンペーン', '広告レポート_キャンペーン別', 'キャンペーン別')}
          onPptx={() => exportBreakdownToPptx(data, '広告レポート - キャンペーン別', 'キャンペーン', '広告レポート_キャンペーン別')}
        />
      </div>
      <div className="bg-card-bg rounded-xl border border-border overflow-hidden mb-8">
        <DataTable data={data} labelHeader="キャンペーン" />
      </div>
